...
int main(int argc, char **argv)
{
	...
	*/
	...
	int numFaces_new;
	if(station_num==2){
		numFaces_new=numFaces_A2_drop;
	}
	else if(station_num==3){
		numFaces_new=numFaces_A3_drop;
	}
	cout<<"Num faces new is "<<numFaces_new<<endl;
	...
	for(Long64_t event=0;event<numEntries;event++) {
		...
		if (isSimulation == true){
			...
			while (foundNextSimEvent == false){
				...
				global_trig=reportPtr->stations[0].Global_Pass;
		...
		if (analyzeEvent == true){
			...
			cout<<"Faces alternate size "<<faces_alternate[1].size()<<endl;
