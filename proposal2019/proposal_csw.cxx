...
void makeCSW(int station, int runNum, int eventNumber, int thetaIn, int phiIn);
	...
	for(int file_num=3; file_num<argc; file_num++){
		...
		if(numHPolIsol==1 && numVPolIsol==0 && numCombo==0){
			...
			for(int event=0; event<numEntries; event++){
				...
				printf("Run %d, Event %d, Theta %d, Phi %d \n", runNum, eventNumber, theta_300[1], phi_300[1]);
				cout<<"About to call the CSW maker "<<endl;
				makeCSW(station, runNum, eventNumber, theta_300[1], phi_300[1]);
...
void makeCSW(int station, int runNum, int eventNumber, int thetaIn, int phiIn){
	...
	*/
	...
	char title_txt[200];
	sprintf(title_txt,"/users/PAS0654/osu0673/A23_analysis_new2/AraRoot/analysis/a23_analysis_programs/proposal2019/A%d_run%d_ev%d.csv",station,runNum,eventNumber);
	...
	sprintf(save_temp_title, "/users/PAS0654/osu0673/A23_analysis_new2/AraRoot/analysis/a23_analysis_programs/proposal2019/A%d_Run%d_Even%d_Waveforms.png",station, runNum, eventNumber);
	...
	cstaggered->cd(2);
		...
		CSW->Draw("sameL");
		CSW->SetLineWidth(2);
	sprintf(save_temp_title, "/users/PAS0654/osu0673/A23_analysis_new2/AraRoot/analysis/a23_analysis_programs/proposal2019/A%d_Run%d_Even%d_WaveformsTimeTranslated.png",station, runNum, eventNumber);
