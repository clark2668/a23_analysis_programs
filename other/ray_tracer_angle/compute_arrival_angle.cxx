...
int main(int argc, char **argv)
{
	...
	// set up a target, choose channel 0 for now
	Position target;
	...
	/*
		// this *should* be the output I think...
		[0][X] = path length
		[1][X] = launch angle
		[2][X] = receipt angle
	...
	*/
	...
	for(double thetaScan=0.; thetaScan<90.; thetaScan+=1.){
		...
		Double_t xs = R*TMath::Cos(this_thetaWave)*TMath::Cos(phiWave);
		Double_t ys = R*TMath::Cos(this_thetaWave)*TMath::Sin(phiWave);
		Double_t zs = R*TMath::Sin(this_thetaWave);
