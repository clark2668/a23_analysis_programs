////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////
////	v2_analysis_save_vals_100_A3.cxx 
////	A23 diffuse, save values for cuts for 100pct sample
////	Should rely on "reco" files and "CWID" files only
////	upgraded specifically to support some changes we see in A3
////	there's one other major change, which is that we now load the data file "ahead of time"
////	and recheck all the quality cuts on some events for consistency
////	and brian caught, at the 11th hour, the mistake with not correctly importing wfrms info for A3
////
////	Aug 2019
////////////////////////////////////////////////////////////////////////////////
...
int main(int argc, char **argv)
{
	...
	if(isSimulation){
		cout<<"This code is pretty heavily tuned for data only, why are you trying to run this on simulation?"<<endl;
		return -1;
	}
	...
	for(int file_num=14; file_num<argc; file_num++){
		...
		// we're also almost certainly going to need the data file, so let's just load that now too... this is new for A3
		...
		if(!eventTree){
			cout<<"Can't find eventTree"<<endl;
			return -1;
		}

		RawAtriStationEvent *rawPtr =0;
		eventTree->SetBranchAddress("event",&rawPtr);
		int numEntriesOrgDataFile=eventTree->GetEntries();
		// numEntriesOrgDataFile=700;
		...
		int theta_41_new[2];
		int phi_41_new[2];
		int surfTopTheta[2]; // ForA3
		int surfTopPhi[2]; // ForA3
		...
		trees[0]->Branch("surf_top_V_theta",&surfTopTheta[0]); // ForA3
		trees[0]->Branch("surf_top_V_phi",&surfTopPhi[0]); // ForA3
		...
		trees[1]->Branch("surf_top_H_theta",&surfTopTheta[1]); // ForA3
		trees[1]->Branch("surf_top_H_phi",&surfTopPhi[1]); // ForA3
		...
		// numEntries=700;
		if(numEntries!=numEntriesOrgDataFile){
			printf("The number of entries in the original data file (%d) does not equal the number of entries in the filter tree of the 100pct reco file (%d). Abort!\n", numEntriesOrgDataFile, numEntries);
			return -1;
		}
		...
		for(int event=start; event<numEntries; event++){
			...
			for(int pol=0; pol<2; pol++){
				...
				surfTopTheta[pol]=-1000; // ForA3
				surfTopPhi[pol]=-1000; // ForA3
				WFRMS_org[pol]=0;
				WFRMS_new[pol]=0;
				for(int i=0; i<8; i++){
			...
			// if(WFRMS_org[0]==0 || WFRMS_org[1]==0)
			// 	printf("Event %d: best face V %.2f, best face H %.2f , does it fail V %d, does it fail H %d \n", eventNumber_out, log(bestFaceRMS[0])/log(10), log(bestFaceRMS[1])/log(10), WFRMS_new[0], WFRMS_new[1]);

			// continue;

			// we're gonna move this out here, which will essentially allow us to check for quality on *every* event, as opposed to only on some
				...
				if(!isCalPulser_in
					...
					*/
					...
					) 
					{
						...
						surfTopPhi[pol]=PeakPhi_Recompute_300m_top; // ForA3
						surfTopTheta[pol]=PeakTheta_Recompute_300m_top; // ForA3
