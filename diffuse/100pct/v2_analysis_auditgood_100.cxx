...
////	this time save out the list that is "good" so we can analyze them
////	the thing we will save out is the location of the "reco" file (which is what we need to make cuts)
////	and sorted by config already, which is supported by a new function in the tools class
////
////	Aug 2019
////////////////////////////////////////////////////////////////////////////////
...
int main(int argc, char **argv)
{
	...
	*/
	...
	cout<<"The config is "<<config<<endl;

	// first, the filter file
	char filename_filter[400];
	sprintf(filename_filter,"%s/A%d/by_config/c%d/processed_station_%d_run_%d_filter.root",file_location_filter.c_str(),station,config,station,runNum);
	...
	sprintf(filename_reco,"%s/A%d/by_config/c%d/processed_station_%d_run_%d_reco.root",file_location_reco.c_str(),station,config,station,runNum);
	...
	sprintf(filename_cw,"%s/A%d/by_config/c%d/CWID_station_%d_run_%d.root",file_location_cw.c_str(),station,config,station,runNum);
	...
	if(
		...
		sprintf(logfile_good,"/home/brianclark/A23_analysis_new2/a23_analysis_scripts/wipac_scripts/100pct/step5-audit/good_A%d_c%d.txt",station,config);
