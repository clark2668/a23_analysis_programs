...
double getSignal(int station, double Sbin1, double Sbin2, double Sbin3, double Sbin4, double Sbin5){
	
	if(station==2){
		Sbin1*=live_frac_A2[0];
		Sbin2*=live_frac_A2[1]*2.; //because Brian was dumb and threw half as many neutrinos in config 2 and config 3
		Sbin3*=live_frac_A2[2]*2.; //because Brian was dumb and threw half as many neutrinos in config 2 and config 3
		Sbin4*=live_frac_A2[3];
		Sbin5*=live_frac_A2[4];
	}
	else if(station==3){
		Sbin1*=live_frac_A3[0];
		Sbin2*=live_frac_A3[1];
		Sbin3*=live_frac_A3[2];
		Sbin4*=live_frac_A3[3];
		Sbin5*=live_frac_A3[4];
	}
...
int main(int argc, char **argv)
{
	...
	if(doTest){
		...
		cVerify->cd(7);
			...
			gPad->SetLogy();
		char save_title[150];
		sprintf(save_title,"validate_new_grid_search_A%d.png");
		cVerify->SaveAs(save_title);
	...
	if(allConverge && allGood){
		...
		for(int config=0; config<5; config++){
			printf("Start bin for slope %.2f and config %d is bin %d and intercept %.2f with signal %.2f \n", slope, config,start_bins[config],intercept[start_bins[config]], signal_all[config][start_bins[config]]);
		}
	}
	...
	if(allConverge && allGood){
		...
		for(int bin1=start_bins[0]+0; bin1<start_bins[0]+12; bin1+=2){ // loop over config 1 bins
			...
			for(int bin2=start_bins[1]+0; bin2<start_bins[1]+12; bin2+=2){ // loop over config 2 bins
				if(bin2>numBins) continue;
				...
				for(int bin3=start_bins[2]+0; bin3<start_bins[2]+12; bin3+=2){ // loop over config 3 bins
					if(bin3>numBins) continue;
					...
					for(int bin4=start_bins[3]+0; bin4<start_bins[3]+12; bin4+=2){ // loop over config 4 bins
						if(bin4>numBins) continue;
						...
						for(int bin5=start_bins[4]+0; bin5<start_bins[4]+12; bin5+=2){ // loop over config 5 bins
							// cout<<"     Slope "<<slope<<" on bin5 bin "<<bin5<<" which is intercept "<<intercept[bin5]<<endl;
							if(bin5>numBins) continue;
							...
							double total_signal = getSignal(station, signal_all[0][bin1],signal_all[1][bin2],signal_all[2][bin3],signal_all[3][bin4],signal_all[4][bin5]); // this function livetime adjusts the flux and fixes a numerical mistake brian made
							...
							if(this_S_over_Sup>best_S_over_Sup){
								...
								best_bins[3]=bin4;
								best_bins[4]=bin5;
								printf(" Slope %.2f has a new optimum S/Sup of %.3f and intercepts %.2f, %.2f, %.2f, %.2f, %.2f at bins %d, %d, %d, %d, %d \n",
											slope,
											best_S_over_Sup, 
	...
	sprintf(outfile_name,"/users/PAS0654/osu0673/A23_analysis_new2/optimize/A%d_optimize_pol%d_new_shorter_jump2.txt",station,pol_select);
