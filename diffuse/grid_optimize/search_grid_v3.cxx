...
int main(int argc, char **argv)
{
	...
	if(allConverge && allGood){
		...
		for(int bin1=start_bins[0]-2; bin1<start_bins[0]+10; bin1++){ // loop over config 1 bins
			...
			for(int bin2=start_bins[1]-2; bin2<start_bins[1]+10; bin2++){ // loop over config 2 bins
				...
				for(int bin3=start_bins[2]-2; bin3<start_bins[2]+10; bin3++){ // loop over config 3 bins
					...
					if(back3>1. || back3<1e-6) continue; // no background this large/small in single bin will ever be viable as part of final answer
						...
						for(int bin4=start_bins[3]-2; bin4<start_bins[3]+10; bin4++){ // loop over config 4 bins
							...
							if(back4>1. || back4<1e-6) continue; // no background this large/small in single bin will ever be viable as part of final answer
								...
								for(int bin5=start_bins[4]-2; bin5<start_bins[4]+10; bin5++){ // loop over config 5 bins
									...
									double this_Sup;
									double total_back = back1+back2+back3+back4+back5;

									// don't waste time (and not a small amount of time, mind you) actually computing Sup
									// if we know the answer is going to be 2.3 anyway 
									// because the background is so far below zero
									if(total_back<1e-4){
										this_Sup=2.3;
									}
									else{
