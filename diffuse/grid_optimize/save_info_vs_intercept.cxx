...
int Optimize(int station, int config, int pol_select, double slope, int numBins_in, double intercepts_in[200], double (&background_out)[200], double (&signal_out)[200], double (&fitPar_out)[2], double (&fitParErr_out)[2], double &pval_out);
...
int main(int argc, char **argv)
{
	...
	for(int this_config=1; this_config<6; this_config++){
		...
		fitConverge = Optimize(station,this_config, pol_select, slope, 200, intercept, backgrounds, signal, fitPar, fitParErr, pVal);
		OutputTree->Fill();
	}
	OutputFile->Write();
	OutputFile->Close();
