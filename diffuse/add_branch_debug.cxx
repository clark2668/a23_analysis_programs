...
int main(int argc, char **argv){
    ...
    //
		...
		for(int event=0; event<numEntries; event++){
			...
			// cout << evt_num << endl;
			// cout<<"Event "<<evt_num<<" has eventId "<<rawEvPtr->eventId<<endl;
			...
			// for(int i=0; i<5; i++){
			// 	cout<<"Sample "<<i<<" has x value "<<graphs[0]->GetX()[i]<<endl;
			// }
			...
			// printf("Event %5d, isPayloadBlast? %d \n", rawEvPtr->eventNumber,isPayloadBlast);
			if(isPayloadBlast){
				numBlast++;
			}
		...
		printf("Num blast / num total %5d/%5d = %.3f \n", numBlast, numTotal, double(numBlast)/double(numTotal));
	}
}
