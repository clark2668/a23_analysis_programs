...
int main(int argc, char **argv)
{
	...
	double background_estimate;
	double unscaled_signal;
	double optminal_s_over_sup;
	double p_value;

	int successful_optimization = Optimize(station, config, pol_select, slope, background_estimate, unscaled_signal, optminal_s_over_sup, p_value, true);
	if(successful_optimization!=0)
		printf("Something went wrong with the optimization in station %d, config %d, pol %d \n", station, config, pol_select);
}
...
int Optimize(int station, int config, int pol_select, double slope, double &background_out, double &signal_out, double &s_over_sup_out, double &pval_out, bool doPrint){
	...
	char the_data[500];
	sprintf(the_data,"/fs/scratch/PAS0654/ara/10pct/ValsForCuts/A%d/c%d/cutvals_drop_snrbins_0_0_wfrmsvals_-1.3_-1.4_run_2*.root",station,config);
	...
	*/
	...
	char the_sims[500];
	sprintf(the_sims,"/fs/scratch/PAS0654/ara/sim/ValsForCuts/A%d/c%d/E%d/cutvals_drop_snrbins_0_0_wfrmsvals_-1.3_-1.4_run_1*.root",station,config,224);
