...
int main(int argc, char **argv)
{
	...
	if(argc<3){
		cout<< "Usage\n" << argv[0] << " <station> <output_location> <joined filename 1> <joined filename 2 > ... <joined filename x>"<<endl;
		return 0;
	}
	...
	for(int file_num=3; file_num<argc; file_num++){
		...
		for(int event=0; event<numEntries; event++){
			...
			for(int pol=0; pol<2; pol++){
				...
				// now, if the event is mis-reconstructing badly, let's check to see if it has CW
				if(pol==0){
					bool isCP5=false;
					bool isCP6=false;
